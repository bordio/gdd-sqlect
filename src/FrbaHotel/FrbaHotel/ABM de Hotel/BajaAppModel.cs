    ...
    class BajaAppModel
    {
        ...
        public bool validar_establecimiento_vacio(Control Desde, Control Hasta)
        {
            ...
            if (Desde.Text.Length > 0 && Hasta.Text.Length > 0)
            {
                ...
                sentece.AppendFormat("SELECT id_reserva FROM SQLECT.Reservas WHERE NOT (estado_reserva in (2,3,4)) AND fecha_inicio BETWEEN '{0}' AND '{1}' OR fecha_inicio+cant_noches_reserva BETWEEN '{0}' AND '{1}'", fecha_desde, fecha_hasta);
